	...
	// }).getDict()).setFlushUI(function(json, append) {
		...
		$("#rankLZ .select").off("click").change(function() { 
			...
			if (rank == 1) {
				...
				// 	console.log("____ realname="+json[iii].realname + " id=" + json[iii].id  + " suggest="+json[iii].suggest)
				// }
			} else if (rank == 7) {
				json.sort((a, b) => b.specialsurvey - a.specialsurvey)
				rank_sort_fifty(json,rank,append);
...
$("#noticebulletin nav>a").off("click").click(function() {
	...
	var ind = $(this).index() + 1;
	noticeSelectTab = ind ;
	console.log("____________ aaaaaaaaaaaaa",ind)
	var messageJson = [];
...
function plusReady() {
	console.log("__________________ aaaaa plusReady")
	plus.runtime.setBadgeNumber(badge_number)
	// 监听“返回”按钮事件
	plus.key.addEventListener("backbutton", function() {
		// console.log("__________________ plusReady")
