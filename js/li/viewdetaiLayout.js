$("#viewdetaiLayout").load(function () {
	var key = $("#viewdetaiLayout_id").val();
	var typeid = $("#viewdetaiLayout_typeid").val();
	var tablename = $("#viewdetaiLayout_tablename").val();
	console.log(" ___________________ ans key=",key)

	if (arry.indexOf("viewdetaiLayout") == "-1") {
		$("#viewdetaiLayout ul li").eq(0).siblings().remove();
		arry.push("viewdetaiLayout")
	} else {
		$("#viewdetaiLayout ul li").remove();
	}
	
	addInformation ( tablename , key , typeid )
...
//查看视察调研的具体内容
function addInformation( tablename , key , typeid ) {
	var Obj = "viewdetaiLayout";
